...
export default {
  data: () => ({
    ...
    dialogYearSelect: false,
  }),
  computed: {
    titleYear() {
      return moment(this.value).format('yyyy年')
    },
    titleMonth() {
      return moment(this.value).format(' M月')
    },
  },
  methods: {
    setToday() {
    ...
    yearSelect(year) {
      ...
      this.value = d.format('yyyy-MM-DD') // 書式を戻す
    },
  },
}
</script>
